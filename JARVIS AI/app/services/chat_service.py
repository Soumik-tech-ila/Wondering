...
sessions (each identifies by session id).
...
RESPONSIBILITIES:
  ...
  - format_history_for llm: Turn the message list into (user, assistant) pairs
    and trim to MIX_CHAT_HISTORY_TURnS so don't overflow the prompt.
  - process_message /process_realtime_message: Add user message, call groq (or 
    realtimeGroq) , add assistant reply, retun reply. 
  - save_chat_session: Write session TO database/chat_data/*.json so it persists
    and can be loaded on next startup (and used by vector store the retrieval).
"""
...
class ChatService:
    ...
    def load_session_from_disk(self, session_id: str) -> bool:
        """
        load a session from databaasechats_data/ if a file for this session_id exists.
        ...
        """
        #sanatise ID for use in filename (no dashes or spaces).
    ...
    def validate_Session_id(self, session_id: str) -> bool:
        """
        Returns True if session_id is safe to use(non-empty, no path transversal, length <= 255).
        Used to reject malicious or invalid IDs before we use them in file paths.
        """
        if not session_id or not session_id.strip():
            return False      
        # Block path transversal and path separators.       
    ...
    def get_chat_history(self, session_id: str) -> List[ChatMessage]:
            """Return the list of messages for this session(chromological). Empty list if session unknown."""
            return self.sessions.get(session_id, [])
        
    def format_history_for_llm(self, session_id: str, exclude_last: bool = False) -> List[tuple]:
        """
        Bulid a list of (user_text, assistant_ text) pairs for the LLM prompt.
        
        WE only include complete pairs and cap at MAX_CHAT_HISTORY_TURNS (e.g. 70)
    ...
    def process_message(self, session_id: str, user_message: str) -> str:
        ...
        response = self.groq_service.get_reponse(questions=user_message, chat_history=chat_history)
    ...
    def process_realtime_message(self, session_id: str, user_message: str) -> str:
        ...
        User the same session as process_message so history is shared. Raises ValueError if realtime_service is None. 
        """
    ...
    def save_chat_session(self, session_id: str):
        """
        Write this session's messages to database/chat_data/chat_{safe_id}.json.
        
        Called after each message so conversation is persisted. The vector store
        is rebuild on startup from this files, so new chats are included after restart.
        If the session is mission or empty we do nothing. On write error we only log.
        """
